...
public class AdminController extends HttpServlet {
        ...
        switch (action) {
            ...
            case "crear":
                if (session.getAttribute("usuario") == null) {
                    ...
                    rd = request.getRequestDispatcher("/frmcoche.jsp");
                    rd.forward(request, response);
                }
                break;
            case "eliminar":
    ...
    private void eliminarServidor(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        int idServidorParam = Integer.parseInt(request.getParameter("idCoche"));
        DbConnection conn = new DbConnection();
        CocheDao cocheDao = new CocheDao(conn);
        int respuesta = cocheDao.delete(idServidorParam);
        String msg = "";
        if (respuesta == 1) { // Fue afectado un registro, esto significa que si se borro
            msg = "El coche fue eliminado correctamente.";
        } else {
            msg = "Ocurrio un error. El coche no fue eliminado.";
        }
        conn.disconnect();
        request.setAttribute("message", msg);
        RequestDispatcher rd;
